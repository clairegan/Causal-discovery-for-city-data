...
import numpy as np
from PCMCI_urban.CMI_estimator import CMI_estimator
# from cmi_kernel import CMI_KERNEL
...
class pcmcim(PCMCIbase):
    def __init__(self, array,var_names,
                #  tau_min, tau_max,
        ...
        self.array = array
        self.M, self.N, self.T = array.shape # N: dimemsion
        self.sig_samples = sig_samples
        self.shuffle_neighbors = shuffle_neighbors
        self.knn = knn
        # self.tau_min = tau_min
        # self.tau_max = tau_max
        self.cmi = CMI_estimator
        self.verbosity = 0
        # self.cmi_estimator = CMI_estimator(array=self.array, sig_samples=self.sig_samples,shuffle_neighbors=self.shuffle_neighbors, knn=self.knn)
    ...
    def _iter_indep_conds(self, _int_parents, _int_link_assumptions, max_conds_py,max_conds_px):
    ...
    def _set_link_assumptions(self, link_assumptions, 
                              tau_min, tau_max,
    ...
    def _set_max_condition_dim(self, tau_min, tau_max, max_conds_dim):
        """
    ...
    def get_lagged_dependencies(self,
        ...
        print("\n##\n## Estimating lagged dependencies \n##"
                "\n\nParameters:")
        print("\nindependence test = %s" % CMI_estimator
                + "\ntau_min = %d" % tau_min
                + "\ntau_max = %d" % tau_max)
    ...
    def _print_a_pc_result(self, nonsig, conds_dim, max_combinations):
        ...
        # Print the message
        print(print_str)
        print(nonsig, conds_dim, max_combinations)
    ...
    def _get_single_residuals(self, array, xyz, target_var,
        ...
        y = np.fastCopyAndTranspose(array[xyz[target_var], :])

        if dim_z > 0:
            z = np.fastCopyAndTranspose(array[xyz[2], :])
    ...
    def parcorr_dependence_measure(self, array, xyz):
        ...
        Parameters
        ----------
        array : array-like, array_reshaped
            data array with X, Y, Z in rows and observations in columns
        ...
        corr, val = self.mult_corr(array_resid)
    ...
    def parcorr_shuffle_significance(self, array, xyz, value,
        ...
        null_dist = self.cmi._get_shuffle_dist(array_resid, xyz_resid,
                                           self.parcorr_dependence_measure,
    ...
    def partial_corr(self, array, X, Y, Z):
        ...
        array_shift, _ = self.cmi._array_reshape(self, array, X=X, Y=Y, Z=Z)
            ...
            for index_parent, parent in enumerate(parents):
                ...
                for comb_index, Z in \
                        enumerate(self._iter_conditions(parent, conds_dim,
                                                        parents)):
                # for Z in [[(0, -t) for t in range(1, tau+1)] for tau in range(tau_min, tau_max+1)]:
                    ...
                    if link_assumptions_j[parent] == '-->':
                        ...
                        print("X: ",parent,"Y: ",j, "Z: ", Z)
                        # val = self.cmi(array=self.array, sig_samples=self.sig_samples, shuffle_neighbors=self.shuffle_neighbors, knn=self.knn).independence_measure(self.array, 
                        #                                                  X = [parent], Y = [(j, 0)], Z = Z)
                        # print("CMI: ",np.round(val, 5),)

                        # pval = self.cmi(array=self.array, sig_samples=self.sig_samples, shuffle_neighbors=self.shuffle_neighbors, knn=self.knn).get_shuffle_significance(X = [parent], Y = [(j, 0)], Z = Z, value= val)
                        # pval = self.cmi(array=self.array, sig_samples=self.sig_samples, shuffle_neighbors=self.shuffle_neighbors, knn=self.knn).get_shuffle_significance(X = [parent], Y = [(j, 0)], Z = Z,
                        #                                                     value=val)

                        # val, pval = self.partial_corr(array=self.array,
                        #                               X = [parent], Y = [(j, 0)], Z = Z )
                        cmi_estimator = self.cmi(array=self.array, sig_samples=self.sig_samples,shuffle_neighbors=self.shuffle_neighbors, knn=self.knn)
                        val = cmi_estimator.cmi_symb(array=self.array,  X = [parent], Y = [(j, 0)], Z = Z)
                        pval = cmi_estimator.symb_parallel_shuffles_significance(X = [parent],Y = [(j, 0)], Z = Z,value=val)

                        # val = cmi_estimator.independence_measure(array = self.array, X = [parent], Y = [(j, 0)], Z = Z)
                        # pval = cmi_estimator.parallel_shuffles_significance(X = [parent], Y = [(j, 0)], Z = Z, value = val)

                        # CMI_KNN(self.array,time_lag=abs(tau)).independence_measure(X=i,Y=j,Z=Z)
                        if pval < pc_alpha:
                            dependent = True
                    ...
                    print('cmi_value: ',val, "cmi_pval: ", pval)
    ...
    def run_pc_stable(self,
        ...
        # Return the parents
        np.save('parent_set.npy',all_parents)
        return all_parents
    ...
    def _run_mci_or_variants(self,
        ...
        #     conf_matrix = np.zeros((self.N, self.N, self.tau_max + 1, 2))
        ...
        for j, i, tau, Z in self._iter_indep_conds(_int_parents,
            ...
            if val_only is False:
                ...
                # print(val, pval)
                    ...
                    # val, pval = self.partial_corr(array=self.array,
                    #                                   X = X, Y = Y, Z = Z )
                    val = cmi_estimator.cmi_symb(array=self.array, X = X, Y = Y, Z = Z)
                    pval = cmi_estimator.symb_parallel_shuffles_significance(X,Y,Z, value=val)

                    # val = cmi_estimator.independence_measure(array = self.array, X = X, Y = Y, Z = Z)
                    # pval = cmi_estimator.parallel_shuffles_significance(X = X, Y = Y, Z = Z, value = val)

                val_matrix[i, j, abs(tau)] = val
                p_matrix[i, j, abs(tau)] = pval
            else:
                val = self.cmi(self.array,time_lag=abs(tau)).independence_measure(self.array, X = X, Y = Y, Z = Z)
                val_matrix[i, j, abs(tau)] = val 
            print(val, pval) 

        # np.save('mci_pmatrix.npy', p_matrix)                                                                                                                         
        # Test significance
        ...
        # Print the results
        # if self.verbosity > 0:
        # self.print_results(results, alpha_level=alpha_level)
    ...
    def run_pcmci(self,
        ...
        # Print the results
        # if self.verbosity > 0:
        self.print_results(results, alpha_level=pc_alpha)
    ...
    def _run_pcalg_test(self, graph, i, abstau, j, S, lagged_parents, max_conds_py,
        ...
        if graph[i,j,abstau] != "" and graph[i,j,abstau][1] == '-':
            ...
            print([(i, -abstau)],[(j, 0)],Z)
            cmi_estimator = self.cmi(array=self.array, sig_samples=self.sig_samples,shuffle_neighbors=self.shuffle_neighbors, knn=self.knn)
            val = cmi_estimator.cmi_symb(array=self.array,  X=[(i, -abstau)], Y=[(j, 0)],
                                                Z=Z, )
            pval = cmi_estimator.symb_parallel_shuffles_significance(X=[(i, -abstau)], Y=[(j, 0)],
                                                Z=Z,value=val)
            
            print('cmi_value: {}, cmi_pval: {}'.format(str(val), str(pval)))
            if pval <= alpha_or_thres:
                dependent = True
            else:
                dependent = False
